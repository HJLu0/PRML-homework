# =============================================================================
# cnn_model.py —— 实验二：引入位置编码的 CNN 序列模型
# 对比对象：Transformer（自注意力）vs CNN 重构网络
# 参考：ConvS2S 思路，使用 Gated Linear Units + 位置编码
# =============================================================================
...
class CNNDecoderLayer(nn.Module):
    """
    解码器层：Masked Conv（因果卷积）+ 与编码器的注意力 + 残差
    为简化实现，注意力部分使用点积注意力（与 ConvS2S 一致）
    """
    def __init__(self, d_model, kernel_size=3, dropout=0.1):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
