...
for _ in range(epochs):
    y_hat = X @ theta
    grad = (2 / len(X)) * X.T @ (y_hat - y_train)
    theta = theta - lr * grad
...
plt.plot(x_line, y_line, color='#9b59b6', linewidth=3, label='拟合曲线')

plt.legend(fontsize=12)
plt.grid(alpha=0.3)
plt.title("梯度下降法", fontsize=14)
plt.savefig("梯度下降.png", dpi=300, bbox_inches='tight')
plt.close()
